...
class CodeGenerator extends Icode {
                                   ...
                                   boolean returnFunction)
    {
        ...
        if (returnFunction) {
            ...
            generateICodeFromTree(scriptOrFn, false);
        }
        return itsData;
    }
    ...
    private void generateFunctionICode()
    {
        ...
        generateICodeFromTree(theFunction.getLastChild(), true);
    }
//modified
    private void generateICodeFromTree(Node tree, boolean isfunction)
    {
        ...
        //Own Code
        if (Token.printgraph && isfunction==false) {
        	LinkedList<Listenelement> lk = new LinkedList<Listenelement>();
        	//InterpreterData in Liste
        	/*for(int ii=0;ii<itsData.getvarStorage().size();ii++){
        		System.out.println(itsData.getvarStorage().get(ii).getposition()+"  "+itsData.getvarStorage().get(ii).getiCode()+"  "+itsData.getvarStorage().get(ii).getvariable());
        	}*/
        	
        	lk = Interpreter.dumpICodeList(itsData,itsData.getvarStorage(),0);
        	//Liste in Objekte
        	mxGraphComponent graphComponent = generateObjectArray(lk,1);
        	//Graphen in Frame
        	graphtoFrame(graphComponent);
        	
        	//Liste in Graph mit Subgraphen
        	//Interpreter.dumpICodeGraph(lk);
        	
        	
        }

    }
    //aus Liste Objekt-Array machen
    private static mxGraphComponent generateObjectArray(LinkedList<Listenelement> lk,int level){
    	//Graphen I
		...
		//Objekte erstellen
		Interpreter.createGraph(lk,parent,graph);
    ...
    public static void graphtoFrame(mxGraphComponent graphComponent){
    	//Frame
          ...
          case Token.FUNCTION:
            {
                ...
                if (!itsInFunctionFlag) {
                  ...
                  //Own Code
                    VariablenListenElement varLE =new VariablenListenElement(); 
                    varLE.setposition(iCodeTop-3);
                    if (fnIndex < 6) {
                    	varLE.setiCode(Icode_REG_IND_C0 - fnIndex);
                    } else if (fnIndex <= 0xFF) {
                    	varLE.setiCode(Icode_REG_IND1);
                     } else if (fnIndex <= 0xFFFF) {
                    	 varLE.setiCode(Icode_REG_IND2);
                     } else {
                    	 varLE.setiCode(Icode_REG_IND4);
                    }
                    varLE.setvariable(node.getString());
                    itsData.getvarStorage().add(varLE);
          ...
          case Token.NEW:
            {
                ...
                if (callType != Node.NON_SPECIALCALL) {
                  ...
                  //Own Code
                    VariablenListenElement varLE =new VariablenListenElement(); 
                    varLE.setposition(iCodeTop-2);
                    if (argCount < 6) {
                    	varLE.setiCode(Icode_REG_IND_C0 - argCount);
                    } else if (argCount <= 0xFF) {
                    	varLE.setiCode(Icode_REG_IND1);
                     } else if (argCount <= 0xFFFF) {
                    	 varLE.setiCode(Icode_REG_IND2);
                     } else {
                    	 varLE.setiCode(Icode_REG_IND4);
                    }
                    try{
                    varLE.setvariable(node.getFirstChild().getString());
                    }
                    catch(Exception e){
                    	varLE.setvariable(" ");
                    }
          ...
          case Token.SETCONST:
            {
              ...
              //Own Code
                VariablenListenElement varLE =new VariablenListenElement(); 
                varLE.setposition(iCodeTop-1);
                varLE.setiCode(Icode_SETCONST);
                varLE.setvariable(child.getFirstChild().getString());
                itsData.getvarStorage().add(varLE);
            }
            break;
          ...
          case Token.SETCONSTVAR:
            {
              ...
              //Own Code
                VariablenListenElement varLE =new VariablenListenElement(); 
                varLE.setposition(iCodeTop-1);
                varLE.setiCode(Icode_SETCONSTVAR1);
                varLE.setvariable(child.getFirstChild().getString());
                itsData.getvarStorage().add(varLE);
            }
            break;
    ...
    private void addStringPrefix(String str)
    {
        ...
        if (index < 4) {
          ...
          //Own Code
            VariablenListenElement varLE =new VariablenListenElement(); 
            varLE.setposition(iCodeTop-1);
            varLE.setiCode(Icode_REG_STR_C0 - index);
            varLE.setvariable(str);
            itsData.getvarStorage().add(varLE);
        } else if (index <= 0xFF) {
          ...
          //Own Code
            VariablenListenElement varLE =new VariablenListenElement(); 
            varLE.setposition(iCodeTop-1);
            varLE.setiCode(Icode_REG_STR_C1 - index);
            varLE.setvariable(str);
            itsData.getvarStorage().add(varLE);
          ...
          //Own Code
            VariablenListenElement varLE =new VariablenListenElement(); 
            varLE.setposition(iCodeTop-1);
            varLE.setiCode(Icode_REG_STR_C2 - index);
            varLE.setvariable(str);
            itsData.getvarStorage().add(varLE);
         } else {
            ...
            VariablenListenElement varLE =new VariablenListenElement(); 
            varLE.setposition(iCodeTop-1);
            varLE.setiCode(Icode_REG_STR4 - index);
            varLE.setvariable(str);
            itsData.getvarStorage().add(varLE);
        }
    }
