...
//         }
    ...
    public String toStringTree(ScriptNode treeTop) {
    	//OwnCode 
        if (Token.printgraph){
        	LinkedList<Listenelement> lk = new LinkedList();
        	toGraphHelper(treeTop, this, null, 0, lk);
        	toGraph(lk);
        }
    	if (Token.printTrees) {
    		StringBuffer sb = new StringBuffer();
    ...
    //OwnCode
    private static void toGraphHelper (ScriptNode treeTop, Node n,
            ObjToIntMap printIds,
            int level, LinkedList<Listenelement> lk){
    	if (printIds == null) {
            printIds = new ObjToIntMap();
            generatePrintIds(treeTop, printIds);
        }
             ...
             cursor = cursor.getNext())
        {
            if (cursor.getType() == Token.FUNCTION) {
                int fnIndex = cursor.getExistingIntProp(Node.FUNCTION_PROP);
                FunctionNode fn = treeTop.getFunctionNode(fnIndex);
                toGraphHelper(fn, fn, null, level + 1, lk);
            } else {
            	toGraphHelper(treeTop, cursor, printIds, level+1, lk);
            }
        }
    }
    
    private static void toGraph(LinkedList<Listenelement> lk){
    	JFrame frame = new JFrame();
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setSize(500, 700);
		frame.setTitle("Kontrollflussgraph auf Rhino´s IR-Code");
		...
		//Ecken
		for(int ii = 1; ii<lk.size();ii++){
			String id = "ID"+ii;
			convertedArgs[ii]= graph.insertVertex(parent, id, lk.get(ii).anweisung, 50*lk.get(ii).level, ii*60-40, 80, 30);
		}	
				
		//geht die Liste zurück bis ein kleines lvl gefunden wird
		for (int ii=2;ii<lk.size();ii++){
			int kk = ii;
			//wenn lvl 1, dann auf letztes lvl 1 stellen, sauberer fluss			
			if(lk.get(ii).level==1){
				int jj=ii-1;
				while(lk.get(jj).level>1){
					jj--;
				}
				kk= jj;
			}
			//sonstige Kanten zu dem vorherigen kleinen lvl
			else{
			while(lk.get(ii).level<=lk.get(kk).level){
				kk--;
			}}
			
			//Kante erstellen
			String id = "ID"+ii;
			graph.insertEdge(parent, id,"" , convertedArgs[kk], convertedArgs[ii]);
		}
