...
import OwnCode.Listenelement;
import OwnCode.VariablenListenElement;
...
// The values that change during interpretation
    ...
    static LinkedList<Listenelement> dumpICodeList(InterpreterData idata, LinkedList<VariablenListenElement> varStorage, int functionCount)
    {
        if (!Token.printgraph) {
            return null;
        }

        LinkedList<Listenelement> lk = new LinkedList<Listenelement>();
        ...
        for (int pc = 0; pc < iCodeLength; ) {
            ...
            switch (token) {
              default:
                ...
                //Vaiablen demaskieren
                if(token==Icode_REG_STR_C0||token==Icode_REG_STR_C1||token==Icode_REG_STR_C2||token==Icode_REG_STR_C3){
                for(int kk=0;kk< varStorage.size(); kk++){
                	if(pc-1==varStorage.get(kk).getposition()){
                		le.anweisung=tname +"  /Variable: "+varStorage.get(kk).getvariable();
                		break;
                	}
                }}
               else{
              ...
              case Icode_REF_INC_DEC: {
                int incrDecrType = iCode[pc];
                le.anweisung = tname + " " + incrDecrType;
                ++pc;
                break;
              }
              ...
              case Icode_CALLSPECIAL : {
                ...
                boolean isNew =  (iCode[pc + 1] != 0);
                int line = getIndex(iCode, pc+2);
                le.anweisung = tname+" "+callType+" "+isNew+" "+indexReg+" "+line;
                pc += 4;
                break;
              }
              ...
              case Token.CATCH_SCOPE:
                {
                    boolean afterFisrtFlag =  (iCode[pc] != 0);
                    le.anweisung = tname+" "+afterFisrtFlag;
                    ++pc;
                }
                break;
              case Token.REGEXP :
            	  le.anweisung = tname+" "+idata.itsRegExpLiterals[indexReg];
                break;
              case Token.OBJECTLIT :
              case Icode_SPARE_ARRAYLIT :
            	  le.anweisung = tname+" "+idata.literalIds[indexReg];
                break;
                //Funktionen u.ä. werden erneut augegeben
              case Icode_CLOSURE_EXPR :
              case Icode_CLOSURE_STMT :
            	  le.anweisung = tname+" "+idata.itsNestedFunctions[indexReg];
            	  le.funktionsliste = dumpICodeList(idata.itsNestedFunctions[indexReg], idata.itsNestedFunctions[functionCount].getvarStorage(),functionCount++);
                break;
              case Token.CALL :
              case Icode_TAIL_CALL :
              case Token.REF_CALL :
              case Token.NEW :
            	  le.anweisung =(tname+' '+indexReg);
                break;
              case Token.THROW :
              case Token.YIELD :
              case Icode_GENERATOR :
              ...
              {
                int line = getIndex(iCode, pc);
                le.anweisung =(tname + " : " + line);
                pc += 2;
                break;
              }
              case Icode_SHORTNUMBER : {
                int value = getShort(iCode, pc);
                le.anweisung =(tname + " " + value);
                pc += 2;
                break;
              }
              case Icode_INTNUMBER : {
              ...
              case Icode_SETCONSTVAR1:
            	//Own Code
            	  ...
            	  for(int kk=0; kk<varStorage.size() ;kk++){
            		  if(pc-1==varStorage.get(kk).getposition()){
            			  variable=varStorage.get(kk).getvariable();
            		  }
            	  }
                indexReg = iCode[pc];
                le.anweisung =(tname+" "+indexReg+"  /Variable: "+variable);
                ++pc;
                break;
            }
            if (old_pc + icodeLength != pc) Kit.codeBug();
    ...
    public static void dumpICodeGraph(LinkedList<Listenelement> lk){
		...
		Object[] convertedArgs = new Object[lk.size()];
		//Ecken
		for(int ii = 0; ii<lk.size();ii++){
			convertedArgs[ii]= graph.insertVertex(parent, ""+lk.get(ii).zeile,"["+lk.get(ii).zeile+"] "+lk.get(ii).anweisung, 60, ii*50, 80, 30);
		}
		//Kanten
		int kk;
		for(int ii = 0; ii<lk.size()-1;ii++){
			//System.out.println(lk.get(ii).zeile+" "+lk.get(ii).anweisung);
			//Abarbeitung nach Jumpart, nur nach vorne!
			//Goto - muss besonders behandelt werden
			if(lk.get(ii).anweisung.matches("GOTO.*")){
				//Goto ziel suchen
				for(int jj = 0; jj<lk.size() ;jj++){
					if(lk.get(jj).zeile==lk.get(ii).zielZeile){
						graph.insertEdge(parent, "id","" , convertedArgs[ii], convertedArgs[jj]);
					}
				}
				continue;
			}//IFEQ
			else if(lk.get(ii).anweisung.matches("IFEQ.*")){
				//True-Kante
				for(int jj = 0; jj<lk.size() ;jj++){
					if(lk.get(jj).zeile==lk.get(ii).zielZeile){
						graph.insertEdge(parent, "id","true" , convertedArgs[ii], convertedArgs[jj]);
					}}
					//normale Kante dazu -false
					graph.insertEdge(parent, "id","false" , convertedArgs[ii], convertedArgs[ii+1]);
				
			}//IFNE
			else if(lk.get(ii).anweisung.matches("IFNE.*")){
				//False Kante zum Ziel
				for(int jj = 0; jj<lk.size() ;jj++){
					if(lk.get(jj).zeile==lk.get(ii).zielZeile){
						graph.insertEdge(parent, "id","false" , convertedArgs[ii], convertedArgs[jj]);
					}}
					//normale Kante dazu - true
					graph.insertEdge(parent, "id","true" , convertedArgs[ii], convertedArgs[ii+1]);
			}//GOSUB
			else if(lk.get(ii).anweisung.matches("GOSUB.*")){
				//Gosub Kante zum Ziel
				for(int jj = 0; jj<lk.size() ;jj++){
					if(lk.get(jj).zeile==lk.get(ii).zielZeile){
						graph.insertEdge(parent, "id","gosub" , convertedArgs[ii], convertedArgs[jj]);
					}}
					//normale Kante dazu
					graph.insertEdge(parent, "id","default" , convertedArgs[ii], convertedArgs[ii+1]);
			}
			//default-Kanten
			else{
				//normale Kante, default Vorgang
				graph.insertEdge(parent, "id","default" , convertedArgs[ii], convertedArgs[ii+1]);
			}
		}
        // layout graph, damit Kante herumgeführt
		/* möglich mxParallelEdgeLayout (nein), mxHierarchicalLayout (ok), mxFastOrganicLayout (naja),  
		 * mxCompactTreeLayout(naja), mxFastOrganicLayout(befriedigend),  mxPartitionLayout(nein); mxStackLayout(nein) ,
		 * mxOrthogonalLayout(nein) 
		 */
		...
		//Ecke auf lvl 0 (erste Ecke) in label, da schöner
		//frame.getContentPane().add(new JLabel(lk.get(0).anweisung), BorderLayout.NORTH);
		frame.setVisible(true);  
    }
    
    public static void createGraph(LinkedList<Listenelement> lk, Object parent, mxGraph graph){
    	LinkedList<Object> convertedArgsList = new LinkedList<Object>();
    	//Ecken
    	for(int ii = 0; ii<lk.size();ii++){
    		convertedArgsList.add(graph.insertVertex(parent, ""+lk.get(ii).zeile,"["+lk.get(ii).zeile+"] "+lk.get(ii).anweisung, 60, ii*50, 80, 30));
    		//Funktion
    		if(lk.get(ii).funktionsliste!=null){
    			createGraph(lk.get(ii).funktionsliste,convertedArgsList.get(ii) , graph);
    		}
    	}
    	//Kanten
    	int kk;
    	for(int ii = 0; ii<lk.size()-1;ii++){
    		//System.out.println(lk.get(ii).zeile+" "+lk.get(ii).anweisung);
    		//Abarbeitung nach Jumpart, nur nach vorne!
    		//Goto - muss besonders behandelt werden , Try für leere Anweisung oder ähnliches
    		try{
    		if(lk.get(ii).anweisung.matches("GOTO.*")){
    			//Goto ziel suchen
    			for(int jj = 0; jj<lk.size() ;jj++){
    				if(lk.get(jj).zeile==lk.get(ii).zielZeile){
    					graph.insertEdge(parent, "id","" , convertedArgsList.get(ii), convertedArgsList.get(jj));
    				}
    			}
    			continue;
    		}//IFEQ
    			...
    			for(int jj = 0; jj<lk.size() ;jj++){
    				if(lk.get(jj).zeile==lk.get(ii).zielZeile){
    					graph.insertEdge(parent, "id","true" , convertedArgsList.get(ii), convertedArgsList.get(jj));
    				}}
    				//normale Kante dazu -false
    				graph.insertEdge(parent, "id","false" , convertedArgsList.get(ii), convertedArgsList.get(ii+1));
    			...
    			for(int jj = 0; jj<lk.size() ;jj++){
    				if(lk.get(jj).zeile==lk.get(ii).zielZeile){
    					graph.insertEdge(parent, "id","false" , convertedArgsList.get(ii), convertedArgsList.get(jj));
    				}}
    				//normale Kante dazu - true
    				graph.insertEdge(parent, "id","true" , convertedArgsList.get(ii), convertedArgsList.get(ii+1));
    		}//GOSUB
    		else if(lk.get(ii).anweisung.matches("GOSUB.*")){
    			//Gosub Kante zum Ziel
    			for(int jj = 0; jj<lk.size() ;jj++){
    				if(lk.get(jj).zeile==lk.get(ii).zielZeile){
    					graph.insertEdge(parent, "id","gosub" , convertedArgsList.get(ii), convertedArgsList.get(jj));
    				}}
    				//normale Kante dazu
    				graph.insertEdge(parent, "id","default" , convertedArgsList.get(ii), convertedArgsList.get(ii+1));
    		}//default-Kanten
    		else{
    			//normale Kante, default Vorgang
    			graph.insertEdge(parent, "id","default" , convertedArgsList.get(ii), convertedArgsList.get(ii+1));
